...
export class TopicService {
  ...
  get getTopics() {
    return {
      ...
      'followed': (query?: query) => this.getAllTopics(query),
      ...
      'saved': (query?: query) => this.getAllTopics(query)
    }
  };
  ...
  getFollowedTopics(query = defaultQuery): Observable<ITopic[]> {
    return this.http.get<ITopic[]>(`${url}/c/following${urlQuery(query)}`);
  }
  ...
  getSavedTopics(query = defaultQuery): Observable<ITopic[]> {
    return this.http.get<ITopic[]>(`${url}/c/saved${urlQuery(query)}`);
  }
